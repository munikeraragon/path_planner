...
class Loader:
    ...
    def start_point(self):
        ...
        if self.cartesian:
            ...
            x_coord, y_coord = project(
                starting_coord['longitude'], starting_coord['latitude'])
            x_coord += -8641750
            y_coord += -3725600
    ...
    def project_coordinates(self, coordinates):
        '''
            Projects polar coordinates into cartesian.
        '''
        project = Basemap(width=12000000, height=9000000, projection='lcc',
                          resolution='c', lat_1=45., lat_2=55, lat_0=50, lon_0=-107.)
        x_coords = []
    ...
    def unproject_coordinates(self, path):
        '''
            Project cartesian coordinates back into polar
        '''
        if self.cartesian:
            return coordinates
            ...
            for segment in path[1:]:
                for coord in segment:
                    lon, lat = project(
                        coord[0]+8641750, coord[1]+3725600, inverse=True)
                    polar_coords += [[lon, lat]]

            return polar_coords

    def get_coordinates(self, coordinates):
        '''
            Separates coordinate dictionary into an x and y array.
        '''
        x_coords = []
        y_coords = []
